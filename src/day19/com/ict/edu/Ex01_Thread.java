...
//싱글 스레드 : 하나의 프로세스에 하나의 스레드가 일처리 하는 것 (day01~day18에 배운 내용)

//자바에서 스레드 생성법
//	** 자바에서는 start()를 호출하여 run()가 실행되면 스레드 실행 됐다고 함
//	1. Thread 클래스 상속 받기 : start(), run() 메서드가 모두 있다.
//	2. Runnable 인터페이스 구현 : run()만 존재

//Thread 클래스 생성자 : Thread(), Thread(Rannable 인터페이스를 구현한 객체)
//	Thread("스레드이름"), Thread(Rannable 인터페이스를 구현한 객체, "스레드이름")
