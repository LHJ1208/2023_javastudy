...
public class Ex01_class {
	//클래스
	//자바에서의 클래스는 객채(Object)를 정의하는 틀 또는 설계도면과 같은 의미로 사용됨
	...
	//		첫글자 대문자 나머지는 소문자
	//		두 단어 이상일 때는 단어의 첫글자들은 대문자
	//		숫자는 중간인 뒤에 사용, 공백은 사용 불가
	
	//2. 멤버 필드 : 데이터, 상태값, 속성
	//	1) 변수 : 언제든지 변할 수 있는 데이터를 저장하는 공간
	//		첫글자 소문자, 두 단어 이상일 때는 단어의 첫글자들은 대문자
	//		숫자는 중간인 뒤에 사용, 공백은 사용 불가	
	//	2) 상수 : 한번 데이터가 저장되면 변경할 수 없는 데이터를 저장하는 공간
	//		모든 글자가 다 대문자, 단어와 다어 사이에 _(언더바)를 사용할 수 있다.
	
	//3. 멤버 메서드 : 동작, 기능, 실행
	//	- 해당 메서드를 호출(실행)하면 해당 메서드 안에 존재한 내용이 실행됨
	//	- 호출해서 내용이 실행 된 후에는 호출한 곳으로 되돌아 간다.
	//	- 메서드는 다른 메서드를 호출할 수 있음
	//	1) 구성 : [접근제한자] [메서드 종류] 반환형 메서드 이름 ([인자 = 매개변수]) { 실행할 내용 }
	//		가) 접근제한자 : public, protected, 생략(default), private
	//		나) 메서드 종류 : 일반 메서드는 이 부분을 생략한다.
	//			특정 메서드에 해당하는 예약어를 사용 (static, ...)
	//		** 다) 반환형 :
	//			해당 메서드가 실행이 끝나면 호출한 곳으로 되돌아 갈 때 함께 가기고 가는 결과의 자료형
	//			만약에 결과를 가기고 가지 않으면 void라는 예약어를 사용한다.
	//	2) 메서드 이름 :
	//		변수와 같음, 첫글자 소문자, 두 단어 이상일 때 단어 첫글자들은 대문자
	//		숫자는 중간이나 뒤에 사용, 공백은 불가
	//	** 3) 인자 = 매개변수 :
	//		보통은 실행할 때 필요한 정보를 외부에서 받기 위한 통로
	//		생략 가능하고 여러 개 사용 가능
	//	** 4) 오버로딩(중복정의) :
	//		메서드 이름이 같으나 인자 수나 인자 자료형이 다르면 다른 메서드로 판단한다.
	//		즉, 인자의 수나 자료형이 다르면 같은 이름의 메서드가 여러 개가 존재할 수 있다.
	//	** 5)
	//	getter() : 호출하는 입장에서 메서드를 이용해서 정보를 가져오는 메서드
	//	setter() : 호출하는 입장에서 메서드를 이용해서 정보를 변경하는 메서드
	//	변수가 private이면 직접 접근을 할 수 없다. => getter()/setter()을 이용해서 접근한다.
	...
	//	참조변수.멤버필드 또는 참조변수.멤버메서드(), (단, 접근제한자를 확인)
	
	//6. static 과 instance : 멤버 필드, 멤버 메서드에 공통으로 해당하는 사항
	//	1) instance(인스턴스) 필드, 메서드 :
	//		객체가 생성될 때 같이 생성되는 필드와 메서드
	//		일반적인 클래스의 멤버 필드나 멤버 메서드는 모두 인스턴스이다.
	//		호출 방법 : 객체 생성한 다음에 호출
	//			참조변수.멤버필드 또는 참조변수.멤버메서드([인자])
	//			Scanner scan = new Scanner(System.in);
	//			scan.nextInt();
	//		heap 메모리에 만들어진다.
